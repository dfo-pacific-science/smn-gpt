...
SOSA/SSN is a joint W3C/OGC standard (W3C Recommendation 19 Oct 2017). A newer “2023 Edition” is in progress on the W3C Recommendation track as a First Public Working Draft (published 16 Sep 2025). It provides the core observation pattern:

```
┌─────────────┐   sosa:observes   ┌───────────────┐
│   Sensor    │──────────────────▶│  sosa:Property │
└─────────────┘                   └───────────────┘
       │                                    │
...
gcdfo:escapement_count_2024_001 a sosa:Observation ;
    ...
    sosa:phenomenonTime [ a time:Instant ; time:inXSDDate "2024-08-15"^^xsd:date ] ;
    sosa:hasResult [
        a sosa:Result ;
...
Note: the GCDFO constraint IRIs in this example may not exist in the currently published ontology. Treat them as **proposed** constraints unless you can verify them in the ontology you loaded. If a constraint IRI is missing, leave it blank in SDP metadata and propose it via `gpt_proposed_terms.csv`.
...
I-ADOPT Variables can be used as values for `sosa:observedProperty` by typing them as `sosa:Property` (note: `sosa:ObservableProperty` is deprecated in the draft 2023 Edition):
...
The **Darwin Core Data Package (DwC-DP)** is a proposal for a Vocabulary Enhancement to Darwin Core, designed to support richer relational structures than Darwin Core Archives. The Darwin Core Maintenance Group opened public review on **17 September 2025** and extended it through **16 January 2026**. In the proposed model, a key innovation is the **`dwc:Assertion`** class, which generalizes `dwc:MeasurementOrFact`.
