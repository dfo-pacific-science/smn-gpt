...
This pack uses both the shared Salmon Domain Ontology (`smn:`; see `19-salmon-domain-terms.csv`) and the DFO Salmon Ontology (`gcdfo:`; see `07-dfo-salmon-terms.csv`).

Use `smn:` first when a concept is genuinely shared across salmon-domain contexts. Use `gcdfo:` when the concept is explicitly DFO-specific, only available as a DFO bridge/profile term, or needs a DFO-local fallback.

Key idea: **shared identity** should live in `smn:` when that is a good fit, **DFO-local identity** should live in `gcdfo:` when DFO specificity matters, and **external IRIs** are used for alignment, decomposition parts, or broadly reusable concepts.
...
- **Check `smn:` first** for shared salmon-domain concepts; then check `gcdfo:` for DFO-specific or DFO-fallback terms.
...
1. `smn:` **SKOS variable concept** when the variable is genuinely shared across salmon-domain contexts.
2. `gcdfo:` **SKOS variable concept** when the variable is DFO-specific, profile-specific, or only available as a DFO bridge/fallback.
3. External **variable/observable** vocabularies when the concept is genuinely cross-domain (examples from the I-ADOPT catalogue: CF Standard Names, BODC P01, CMO, ICES parameters).
4. As a last resort, a Darwin Core term IRI (e.g., `http://rs.tdwg.org/dwc/terms/individualCount`) only when the column is intended to be *the Darwin Core property* (typical in DwC Archives and `dwc:MeasurementOrFact`, i.e., “a measurement row with `dwc:measurementType` / `dwc:measurementValue` / `dwc:measurementUnit`”). In this case you are pointing at an `rdf:Property`, not a variable concept.

If you use a non-`smn:` / non-`gcdfo:` `term_iri` for a salmon-specific variable, add a proposed shared `smn:` variable concept when the term is broadly reusable, or a `gcdfo:` variable concept when it is truly DFO-specific.

- If the external identifier is a concept/variable IRI, align later with `skos:exactMatch` / `skos:closeMatch`.
- If the external identifier is an RDF property (e.g., a Darwin Core `dwc:` term), treat it as a predicate-level mapping (not `skos:*Match`) and plan to replace `term_iri` with the appropriate `smn:` or `gcdfo:` variable concept when available.
...
1. `smn:` SKOS constraint concept when the constraint is shared across salmon-domain contexts; otherwise `gcdfo:` for DFO-specific life stage, origin, year-basis, or spatial/fishery strata terms.
2. External constraint vocabularies that match the domain (commonly ENVO; CF where appropriate).
3. **AGROVOC** SKOS concepts for broad fisheries/agriculture concepts (reasonable as a lightweight fallback; can be primary for broad, non-salmon-specific constraints; prefer `smn:` / `gcdfo:` for salmon-specific constraints and align to AGROVOC when helpful).
...
gcdfo:WildAdultSockeyeEscapement a iop:Variable , skos:Concept ;
    ...
    iop:hasConstraint gcdfo:WildOrigin ,
                      gcdfo:AdultLifeStage ,
                      gcdfo:EscapementContext .

# Constraints as SKOS concepts
gcdfo:WildOrigin a iop:Constraint , skos:Concept ;
    rdfs:label "Wild origin"@en ;
    skos:definition "Fish that originated from natural spawning, not hatchery production"@en .
